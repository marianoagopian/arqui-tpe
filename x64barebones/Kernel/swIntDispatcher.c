...
unsigned int swIntDispatcher(uint64_t mode, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5) 
{
	switch (mode) {
		case SYS_READ:
			return sys_read((char *) arg0, (unsigned int) arg1);
			
		case SYS_WRITE:
			return sys_write((const char *) arg0,(unsigned int) arg1);
...
unsigned int sys_read(const char *buf, unsigned int count) {
  return readDispatcher(STDIN, buf, count);
}

unsigned int sys_write(const char *buf, unsigned int count) {
  return writeDispatcher(STDOUT, buf, count);
}